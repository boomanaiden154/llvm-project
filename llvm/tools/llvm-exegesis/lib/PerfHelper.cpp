...
Counter::Counter(PerfEvent &&E, std::vector<PerfEvent> &&ValEvents,
		 pid_t ProcessID) : Event(std::move(E)), ValidationEvents(std::move(ValEvents)),
                                    ValidationFileDescriptors(ValidationEvents.size(), -1) {
  assert(Event.valid());
...
void Counter::initRealEvent(pid_t ProcessID) {
  ...
  assert(ValidationFileDescriptors.size() == ValidationEvents.size());
  for (size_t I = 0; I < ValidationEvents.size(); ++I) {
    perf_event_attr AttrCopy = *ValidationEvents[I].attribute();
    ValidationFileDescriptors[I] = perf_event_open(&AttrCopy, ProcessID, Cpu, FileDescriptor, Flags);
    if (ValidationFileDescriptors[I] == -1) {
...
Counter::readValidationCountersOrError() const {
  llvm::SmallVector<int64_t, 4> Result;
  for (const int ValidationFileDescriptor : ValidationFileDescriptors) {
    int64_t Count;
    if (!IsDummyEvent) {
      ssize_t ReadSize = ::read(ValidationFileDescriptor, &Count, sizeof(Count));
      if (ReadSize != sizeof(Count))
        return llvm::make_error<llvm::StringError>("Failed to read validation ounter", llvm::errc::io_error);
    } else {
      Count = -1;
    }
    Result.push_back(Count);
