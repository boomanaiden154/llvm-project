...
ConfiguredEvent::readOrError(StringRef /*unsued*/) const {
  int64_t Count = 0;
...
CounterGroup::CounterGroup(PerfEvent &&E, std::vector<PerfEvent> &&ValEvents,
  ...
  for (auto &&ValidationEvent : ValEvents)
    ValidationCounters.emplace_back(std::move(ValidationEvent));
  if (!IsDummyEvent)
...
void CounterGroup::initRealEvent(pid_t ProcessID) {
  ...
  for (auto &ValCounter : ValidationCounters)
    ValCounter.initRealEvent(ProcessID, EventCounter.getFileDescriptor());
}

void CounterGroup::start() {
  if (!IsDummyEvent)
    ioctl(EventCounter.getFileDescriptor(), PERF_EVENT_IOC_RESET,
          PERF_IOC_FLAG_GROUP);
}

void CounterGroup::stop() {
  if (!IsDummyEvent)
    ioctl(EventCounter.getFileDescriptor(), PERF_EVENT_IOC_DISABLE,
          PERF_IOC_FLAG_GROUP);
}
...
CounterGroup::readValidationCountersOrError() const {
  ...
  for (const auto &ValCounter : ValidationCounters) {
    Expected<SmallVector<int64_t>> ValCounterResultOrErr =
        ValCounter.readOrError(StringRef());
    if (!ValCounterResultOrErr)
      return ValCounterResultOrErr.takeError();

    // Validation Counters only return a single value, thus we can safely push
    // only the first value here.
    Result.push_back((*ValCounterResultOrErr)[0]);
  }
  return Result;
}
